	...
	*/
	function create_trail($sql){
		...
		if($arrayQuery[0]!="SELECT"){
			...
			$table = (isset($pos) && isset($arrayQuery[$pos]))? $arrayQuery[$pos] : null;
			$table=explode("(", $table);
			$table=$table[0];
			$table=explode(" ", $table);
			$table=$table[0];
			
      if(isset($type) && strcmp($table,"SGD_AUDITORIA")!=0 && $type!='') {
        $documento_usuario = (isset($_SESSION['usua_doc']))? $_SESSION['usua_doc'] : null;
				$sql = str_replace("'","^", $sql);
				$ipLocal =  $_SERVER['REMOTE_ADDR'];
				$query="INSERT INTO SGD_AUDITORIA (FECHA,
                                            USUA_DOC,
                                            IP,
                                            TIPO,
                                            TABLA,
