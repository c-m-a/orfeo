	...
	switch(opt.value) {
		case "pys":
			...
			if(window.confirm(mensaje))	{
				urlSancRegist = "{$ENLACE_PAZYSALVO}"+document.form_bdcomple.Ano.value;//+"&";
...
*/
			url = "{$ENLACE_QUEJAYDERECHOCTA}"+quejas;
			url += "{$VARS_QUEJAYDERECHOCTA}";
			url += "&ruta_raiz=..&dependencia={$DEPENDENCIA}<?=$dependencia?>";
...
function abrirSancionados() {	
	...
	urlSancRegist="{$ENLACE_PAZYSALVOSCTA}"+document.form_bdcomple.Ano.value+"&";
...
<form name="form_bdcomple" method="post" action="{$ENLACE_BDCOMPLE}">
