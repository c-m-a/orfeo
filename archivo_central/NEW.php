...
<?php
  ...
  // Funcking silly conditional No one knows how to program in this world Sincerely Cmauricio Parra
  $ver_pestana="";
  /*if($ent==1) {
    $ver_pestana="";
  } else {
    ...
    document.formulario.elements[s].focus();
      ...
      if ((f%4)==0) {
        if(document.formulario.elements[m].value<13) {
          switch(meses) {
            case 12 :
              if(document.formulario.elements[s].value>31) {
              ...
              break;
            case 11 :
              if(document.formulario.elements[s].value>30) {
                alert ("Fecha incorrecta..");
                document.formulario.elements[s].value="";
                document.formulario.elements[m].value="";
                document.formulario.elements[n].value="";
            ...
            case 3 :
              ...
              break;
            case 2 :
              if(document.formulario.elements[s].value>29) {
                alert ("Fecha incorrecta..");
                document.formulario.elements[s].value="";
                document.formulario.elements[m].value="";
                document.formulario.elements[n].value="";
          ...
          if(document.formulario.elements[m].value<13) {
            switch(meses){
              ...
              case 3 :
                ...
                break;
              case 2 :
                if(document.formulario.elements[s].value>28) {
                  alert ("Fecha incorrecta..");
                  document.formulario.elements[s].value="";
                  document.formulario.elements[m].value="";
                  document.formulario.elements[n].value="";
...
<tr>
	...
	?>
      MODULO DE RADICACION ARCHIVO CENTRAL
      <?=$tRadicacionDesc?>
      (Dep
      <?=$dependencia ?>
      ->
...
<table width="99%" border="0" align="center" cellspacing="1" cellpadding="1" class="borde_tab" >
	<tr valign="middle">
		<td class="titulos5" width="15%" align="right">
			...
			<span class="titulos5">A&ntilde;o de Radicaci&oacute;n</span></font>
		</td>
		<td class="listado5" width="15%">
    <font color="" face="Arial, Helvetica, sans-serif">
			<input name="fechproc1" type="hidden" id="fechproc13" size="2" maxlength="2" value="<?php echo $ddate;?>" class="tex_area">
			<input name="fechproc2" type="hidden" id="fechproc23" size="2" maxlength="2" value="<?php echo $mdate;?>" class="tex_area">
      <select name="fechproc3" class="select">
        <?php
          for($i = 1986; $i <= 2006; $i++) 
...
<?php
  for($i=1;$i<=3;$i++) {
    $nombre = $nombre_us{$i};
    $documento = $documento_us{$i};
    $papel = $prim_apel_us{$i};
    $grbNombresUs1 = trim($nombre_us{$i}) . " " . trim($prim_apel_us{$i}) . " ". trim($seg_apel_us{$i});
    $sapel = $seg_apel_us{$i};
    $tel = $telefono_us{$i};
    $dir = $direccion_us{$i};
    $mail = $mail_us{$i};
    ...
    $idc = $idcont{$i};
    $tipo = $tipo_emp_us{$i};
    $cc_documento = $cc_documento_us{$i};
    $otro = $otro_us{$i};
...
if($radi_depe_actu_padre and $tipoanexo==0 and !$coddepeinf)  $coddepe = $radi_depe_actu_padre;
  ...
  $queryWhere = ($ent!=2)? ' where depe_codi=$dependencia' : '';
...
<?php
  ...
  include("../include/tx/radicado_archivo.php");
  include("../class_control/Municipio.php");
  $hist = new Historico($db);
  $Tx = new Tx($db);

  if($Submit3 == 'Radicar') {
    $fecha_radicacion['year'] = $fechproc3;
    $fecha_radicacion['month'] = $hay_mes;
    $fecha_radicacion['day'] = $hay_dia;
    $ddate = date("d");
    $mdate = date("m");
    $adate = date("Y");
  ...
  $numdoc = trim(substr($numdoc,0,13));
    ...
    $noRad = $rad->newRadicado($ent, $tpDepeRad[$ent], $fecha_radicacion);
    if ($noRad=='-1')
...
//$db->conn->debug = true;
		...
		// Se pasa el valor $ruta_raiz a la funci�n informar. Requerido por implementaci�n de alertas. Soluciona el error generado al informar a una
		// dependencia desde el formulario de radicaci�n de entrada.
		//$txSql = $Tx->informar($radicadoSel, $krd,"'".$coddepeinf."'","'".$dependencia."'", $cod_usuario_inf,$codusuario, $observa, $_SESSION['usua_doc']);
