...
$_SESSION['radcom']=trim($rs_datos_radicado->fields['RADI_NUME_RADI']);
$_SESSION['fecha']=trim($rs_datos_radicado->fields['RADI_FECH_RADI']);
$_SESSION['sigla']=trim($rs_datos_radicado->fields['SIGLA_DE_LA_EMPRESA']);
...
$_SESSION['dependencia']=trim($rs_datos_radicado->fields['RADI_DEPE_RADI']);
...
if($rsE = $db->conn->Execute($iSql)){
 $_SESSION['sigla']=trim($rs_datos_radicado->fields['SIGLA_DE_LA_EMPRESA']);
 $_SESSION['espCodigo']=trim($rs_datos_radicado->fields['IDENTIFICADOR_EMPRESA']);
}
...
$mensaje="Su queja fue radicada con exito en el sistema con el n&uacute;mero <font color='red'>".$radicado."</font>, pulse <strong>continuar</strong> para descargar su radicaci&oacute;n en formato PDF. Debe dar click en <font color='red'>continuar</font> para proceder con el tr&aacute;mite ";


	}	
else
	{
		$mensaje="Su queja ya fue radicada en el sistema, si desea radicar una nueva ingrese a nuestra p&aacute;gina Web.<br> Y consulte el radicado $radicado .<a href='http://orfeoweb.supersolidaria.gov.co/orfeo-3.8.0/formularioWeb_ses/'>www.supersolidaria.gov.co</a>";
	}
...
<? if($rs_bus_rad->fields['K']>=0) { ?>
  <tr>
    <td align="center"><input type="button" name="Submit" value="Continuar" onclick="window.location='formulariopdf.php?PHPSESSID=<?=session_id()?>';" />
</td>
  </tr>
  <tr>
