...
<form method="post" action="<?=$encabezadol?>" name="TipoDocu">
  ...
  */
  if ($Actualizar && $tsub !=0 && $codserie !=0 )
  {
     ...
     *  Arreglo con los parámetros del expediente.
	 */
	...
	/**  Procedimiento que Crea el Numero de  Expediente
      ...
      *  Modificado: 09-Junio-2006 Supersolidaria
      *  La función crearExpediente() recibe los parámetros $codiPROC y $arrParametro
	  */
...
<td width="38%" class=listado5 >
 ...
 ?>   
    ...
    /**********************************************/
    // Modificación: 22-Mayo-2006
    // Selecciona el proceso y el código correspondiente según la combinación
    // Serie-Subserie 
...
?> 
	...
	<tr>
        <!--Modificación: 22-Mayo-2006
        Combo para seleccionar el proceso según la combinación Serie-Subserie
        -->
        <!--
        ...
        <td class="listado5" colspan="2" >
          <?
            $comentarioDev = "Muestra los procesos según la combinación Serie-Subserie";
            include "$ruta_raiz/include/tx/ComentarioTx.php";
            ...
            // Si se seleccionó Serie-Subserie-Proceso
            if( $codProc != "" && $codProc != 0 && $codserie != "" && $codserie != 0 && $tsub != "" && $tsub != 0 )
            {
                // Término del proceso seleccionado
...
/******/
  ...
  if(!$anoExp) $anoExp = date("Y");
	...
	<br>
			Año-Dependencia-Serie Subserie-Consecutivo-E<br>
 ...
 <tr align="center">
  ...
  <?php
    ...
    while ( !$rsParExp->EOF )
    {
        // Determina si el parámetro se puede ingersar manualmente.
...
//	$dependencia = '210';
	...
	$sql_Exp = "SELECT * FROM SGD_SEXP_SECEXPEDIENTES 
				WHERE DEPE_CODI = '$dependencia' AND 
				(SGD_SEXP_PAREXP1 = '$nit_ent' OR SGD_SEXP_PAREXP2 = '$sigla' OR SGD_SEXP_PAREXP3 = '$carpeta')";
	...
	<br>
	El (Los) siguiente(s) expediente(s) ya fue(ron) creado(s) y coincide(n) en <br>
	el NIT, SIGLA y/o CARPETA. <br>
	Si está seguro de CREAR el Expediente contin&uacute;e con la Confirmaci&oacute;n.<br>&nbsp;
	</td>
	</tr>
	</table>
	...
	if(!$rs_Exp->EOF){
		...
		<?
		while(!$rs_Exp->EOF){
			$exp_Fecha = $rs_Exp->fields['SGD_SEXP_FECH'];
			$exp_No = $rs_Exp->fields['SGD_EXP_NUMERO'];
			$exp_Nit = $rs_Exp->fields['SGD_SEXP_PAREXP1'];
			$exp_Usu = $rs_Exp->fields['USUA_DOC_RESPONSABLE'];
		...
		No hay Expedientes conincidentes...
		</td>
		<?
		}
		?>
...
<table border=0 width=70% align="center" class="borde_tab">
     ...
     *  No se tiene en cuenta la selección de algún proceso para crear un expediente.
	 */
