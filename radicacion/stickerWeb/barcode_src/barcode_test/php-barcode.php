...
$genbarcode_loc="/usr/bin/barcode";
...
/*barcode_outimage($bars['text'],
                    ...
                    $files_path)
*/
function barcode_outimage($text,
                          $bars,
                          $scale = 1,
                          $mode = "png",
	                        $total_y = 0,
...
function barcode_encode_genbarcode($code,$encoding){
    ...
    $cmd = $genbarcode_loc . ' -E -n -b '	. escapeshellarg($code) . ' ' .
            escapeshellarg(strtoupper($encoding)) . '';
    $text_file = '';
    $text_file = shell_exec($cmd);
    
    $text_explode = explode("\n", $text_file);
    
    // line where the is encoded number
    $bar_code = $text_explode[6];
    $bar_code = ltrim ($bar_code, "% ");
...
function barcode_print($code, $encoding="ANY", $scale = 2 ,$mode = "png", $gen_file = false, $files_name = 'barcode', $files_path = './' ){
    ...
    else barcode_outimage($bars['text'],$bars['bars'],$scale, $mode, $total_y, $space, $gen_file, $files_name, $files_path);
    return $bars;
}
