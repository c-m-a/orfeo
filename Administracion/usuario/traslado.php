...
/* CARLOS ALBERTO BARRERO  carlosabc81@gmail.com 20-04-2010 TRASLADO DE USUARIOS    */
/*                                                          Y SUS RADICADOS ENTRE   */
/*                                                          DEPENDENCIAS            */
/************************************************************************************/
	session_start();
	//$ruta_raiz = '../..';
  include('../../config.php');
  include('../../include/Smarty-3.0b8/libs/Smarty.class.php');
  ...
  //sql trae depenecias destino excepto alcadias
  /*
  $sql_dep_des="SELECT depe_codi,depe_nomb FROM dependencia WHERE depe_codi>20 AND depe_estado=1 ORDER BY 1";
  $rs_dep_des=$db->conn->Execute($sql_dep_des);
  */
  $sqlDep = "select DEPE_CODI,
                    DEPE_NOMB
              from dependencia
              where DEPE_ESTADO='1' order by DEPE_CODI";
?>
<html>
<head>
...
<?php
  ...
  if($_POST['accion']=='1') {
    ...
    $sql_ncodi = "SELECT MAX(USUA_CODI) proximo
                  FROM usuario_tmp
                  WHERE depe_codi=".$_POST['cod_dep_des']; 
    $rs_ncodi  = $db->conn->Execute($sql_ncodi);

    $sql_ncodi2= "SELECT MAX(USUA_CODI) proximo
                  FROM usuario
                  WHERE depe_codi=".$_POST['cod_dep_des']; 
    ...
    $db->conn->debug = true;
    $busqueda_usuario = 'SELECT USUA_LOGIN,
                                USUA_DOC
                           FROM USUARIO
                           WHERE usua_codi = ' . $CODUSUARIO . ' AND
                                 depe_codi = ' . $DEPENDENCIA;

    $rsb = $db->conn->Execute($busqueda_usuario);
    ...
    $usua_codi_n = ($rs_ncodi2->fields['PROXIMO'] != '')?
                      $rs_ncodi2->fields['PROXIMO'] + $rs_ncodi->fields['PROXIMO'] + 1:
                      $rs_ncodi->fields['PROXIMO'] + 1;

    //actualiza perfil con la nueva dependencia
    $update_perfil = "UPDATE usuario_tmp
                      SET usua_codi = $usua_codi_n,
                          depe_codi = $COD_DEP_DES,
                          USUA_CODI_ANTE = $CODUSUARIO,
                          DEPE_CODI_ANTE = $DEPENDENCIA
                      WHERE usua_codi = $CODUSUARIO AND
                            depe_codi = $DEPENDENCIA";
    ...
    $sql_inactiva_usu = "UPDATE usuario
                          SET USUA_ESTA = '0'
                          WHERE usua_codi = " . $CODUSUARIO . " AND
                                depe_codi = " . $DEPENDENCIA;
    ...
    $update_anexos = "UPDATE anexos
                        SET ANEX_CREADOR = 'TMP'
                        WHERE ANEX_CREADOR = '" . $USUA_LOGIN . "'";
    $rs_update_anexo = $db->conn->Execute($update_anexos);
    //echo $rs_update_anexo."<br>";

    //actualiza tabla prestamos
    $update_prestamos = "UPDATE prestamo
                          SET usua_login_actu = 'TMP'
                          WHERE usua_login_actu = '$USUA_LOGIN'";
    $rs_update_prestamos = $db->conn->Execute($update_prestamos);
    //echo $rs_update_prestamos ."<br>";
    
    //cambio login actual en usuarios
    $update_login = "UPDATE usuario
                        SET usua_login = '" . $USUA_LOGIN . $num . "'
                        WHERE usua_login = '$USUA_LOGIN'";
    $rs_update_login = $db->conn->Execute($update_login);
    //echo $update_login."<br>";

    //inserta usuario nuevo
    $ins_nuevo_usu = "INSERT INTO usuario SELECT * FROM usuario_tmp";
    $rs_nuevo_usu  = $db->conn->Execute($ins_nuevo_usu);
    //echo $ins_nuevo_usu."<br>";

    //cambiar usua_login en anexos por login antiguo + fecha
    $update_anexos_old = "UPDATE anexos
                          SET ANEX_CREADOR = '$USUA_LOGIN'
                          WHERE ANEX_CREADOR = 'TMP'";
    ...
    $update_prestamos_old = "UPDATE prestamo 
                              SET usua_login_actu = '$USUA_LOGIN'
                              WHERE usua_login_actu = 'TMP'";
    ...
    $upd_fec = "UPDATE usuario 
                SET usua_fech_crea = SYSDATE
                WHERE depe_codi = $COD_DEP_DES AND
                  usua_codi=".($rs_ncodi->fields['PROXIMO']+1);
    $rs_fec = $db->conn->Execute($upd_fec);
    //echo $upd_fec ."<br>";

    //actualiza carpetas personales al usuario actual en la tabla carpeta_per
    $upd_carpeta = "UPDATE carpeta_per
                    SET usua_codi = ".($rs_ncodi->fields['PROXIMO']+1).",
                        depe_codi = $COD_DEP_DES
                    WHERE usua_codi = $CODUSUARIO AND
                          depe_codi = $DEPENDENCIA";
...
$sql_resultado = "SELECT DECODE(carp_per,0,'SYSTEMA',1,'PERSONAL') \"TIPO DE CARPETA\",
                  (CASE WHEN (carp_per=0 AND carp_codi=0) THEN 'Entradas'
                  ...
                  count(*) RADICADOS
                  FROM radicado
                  WHERE radi_depe_actu = ".$_POST['cod_dep_des']." AND
                        radi_usua_actu=".($rs_ncodi->fields['PROXIMO']+1)."
