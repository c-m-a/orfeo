...
class Radicacion {
 ...
 // Metodo insertar un radicado nuevo
  function newRadicado($tpRad, $tpDepeRad, $date = array()) {
    $year = 0;
    // Busca el Nivel de Base de datos.
		...
		// Busca el usuairo Origen para luego traer sus datos.
		// Busca el usuairo Origen para luego traer sus datos.
		$SecName = "SECR_TP$tpRad" . '_' . $tpDepeRad . '_' . $date['year'];
		...
		$year = $date['year'];
    $newRadicado = $year . $this->dependencia . str_pad($secNew,$this->noDigitosRad,"0", STR_PAD_LEFT) . $tpRad;
    ...
    $recordR["radi_tipo_deri"] = (!$this->radiTipoDeri)? 0 : $this->radiTipoDeri;
		...
		*/
		...
		//$recordR["RADI_FECH_RADI"]="SYSDATE";	
